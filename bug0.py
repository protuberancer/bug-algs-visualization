...
while abs(robotPos[0] - goal[0]) > stepSize or abs(robotPos[1] - goal[1]) > stepSize:
    if not followBoundary:
        ...
        # Проверяем, можно ли двигаться напрямую к цели
        direction_direct = [(goal[0] - robotPos[0]) / np.linalg.norm([goal[0] - robotPos[0], goal[1] - robotPos[1]]),
                            (goal[1] - robotPos[1]) / np.linalg.norm([goal[0] - robotPos[0], goal[1] - robotPos[1]])]
        nextPos_direct = [robotPos[0] + stepSize * direction_direct[0], robotPos[1] + stepSize * direction_direct[1]]

        if not currentObstacle.contains_point(nextPos_direct):
            followBoundary = False
            nextPos = nextPos_direct
            print('Clear path to goal detected, resuming direct navigation.')
        elif at_vertex(robotPos, currentObstacle):  # Проверяем, находится ли робот на вершине
            ...
            while True:
                ...
                # Проверяем, можно ли двигаться напрямую к цели
                direction_direct = [
                    (goal[0] - robotPos[0]) / np.linalg.norm([goal[0] - robotPos[0], goal[1] - robotPos[1]]),
                    (goal[1] - robotPos[1]) / np.linalg.norm([goal[0] - robotPos[0], goal[1] - robotPos[1]])]
                nextPos_direct = [robotPos[0] + stepSize * direction_direct[0],
                                  robotPos[1] + stepSize * direction_direct[1]]
                if not currentObstacle.contains_point(nextPos_direct):
                    followBoundary = False
                    nextPos = nextPos_direct
                    print('Clear path to goal detected, resuming direct navigation.')
                ...
                if hit: break
                if distance_to_obstacle > stepSize: break
                robotPos = nextPos
                ax.plot(robotPos[0], robotPos[1], 'b.')
                plt.pause(0.05)

            if (not hit) and followBoundary:
                ...
                side_vertices = np.array([currentObstacle.vertices[current_side_index],
                                          currentObstacle.vertices[
                                              (current_side_index + 1) % len(currentObstacle.vertices)]])
                ...
                while True:
                    ...
                    if distance_to_vertex > 1.4 * stepSize: break
                    robotPos = nextPos
                    ax.plot(robotPos[0], robotPos[1], 'b.')
                    plt.pause(0.05)
