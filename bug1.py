    ...
    if not followBoundary:
        ...
        for obstacle in obstacles:
            if Path(obstacle).contains_point(nextPos):
                hitPoint = robotPos
                closestPos = hitPoint
                minDist = math.sqrt((hitPoint[0] - goal[0]) ** 2 + (hitPoint[1] - goal[1]) ** 2)
                followBoundary = True
        ...
        if (math.sqrt((robotPos[0] - hitPoint[0]) ** 2 + (robotPos[1] - hitPoint[1]) ** 2)) < 0.99 * stepSize:
            followBoundary = False
            nextPos = closestPos
            ...
            if at_vertex(robotPos, currentObstacle):  # Проверяем, находится ли робот на вершине
                ...
                while True:
                    ...
                    if (
                    math.sqrt((robotPos[0] - hitPoint[0]) ** 2 + (robotPos[1] - hitPoint[1]) ** 2)) < 0.5 * stepSize:
                        followBoundary = False
                        nextPos = closestPos
                        break
                    robotPos = nextPos

                    dist = math.sqrt((robotPos[0] - goal[0]) ** 2 + (robotPos[1] - goal[1]) ** 2)
                    if dist < minDist:
                        minDist = dist
                        closestPos = robotPos
                ...
                if not hit:
                    ...
                    while True:
                        ...
                        if ((math.sqrt((robotPos[0] - hitPoint[0]) ** 2 + (robotPos[1] - hitPoint[1]) ** 2))
                                < 0.5 * stepSize):
                            followBoundary = False
                            nextPos = closestPos
                            break
